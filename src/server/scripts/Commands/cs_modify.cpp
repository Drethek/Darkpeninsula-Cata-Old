...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand modifyCommandTable[] =
        {
            ...
            { "conquest",       SEC_MODERATOR,      false, &HandleModifyConquestCommand,      "", NULL },
            { "justice",        SEC_MODERATOR,      false, &HandleModifyJusticeCommand,       "", NULL },
            { "valor",          SEC_MODERATOR,      false, &HandleModifyValorCommand,         "", NULL },
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        sLog->outDetail(handler->GetDarkCoreString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyHonorCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_HONOR_POINTS, amount*100);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_HONOR, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_HONOR_POINTS)/100);
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetDarkCoreString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
    ...
    static bool HandleModifyConquestCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_CONQUEST_POINTS, amount*100);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_CONQUEST_POINTS)/100);
    ...
    static bool HandleModifyJusticeCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_JUSTICE_POINTS, amount*100);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_JUSTICE_POINTS)/100);
    ...
    static bool HandleModifyValorCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_VALOR_POINTS, amount*100);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_VALOR_POINTS)/100);
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (addmoney < 0)
        {
            ...
            sLog->outDetail(handler->GetDarkCoreString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
            if (newmoney <= 0)
            {
                handler->PSendSysMessage(LANG_YOU_TAKE_ALL_MONEY, handler->GetNameLink(chr).c_str());
                if (handler->needReportToTarget(chr))
                    ChatHandler(chr).PSendSysMessage(LANG_YOURS_ALL_MONEY_GONE, handler->GetNameLink().c_str());
            ...
            else
            {
                ...
                handler->PSendSysMessage(LANG_YOU_TAKE_MONEY, addmoney, handler->GetNameLink(chr).c_str());
                if (handler->needReportToTarget(chr))
                    ChatHandler(chr).PSendSysMessage(LANG_YOURS_MONEY_TAKEN, handler->GetNameLink().c_str(), addmoney);
                chr->SetMoney(newmoney);
            }
        }
        else
        {
            handler->PSendSysMessage(LANG_YOU_GIVE_MONEY, addmoney, handler->GetNameLink(chr).c_str());
            if (handler->needReportToTarget(chr))
                ChatHandler(chr).PSendSysMessage(LANG_YOURS_MONEY_GIVEN, handler->GetNameLink().c_str(), addmoney);
        ...
        sLog->outDetail(handler->GetDarkCoreString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney());
