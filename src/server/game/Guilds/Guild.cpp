...
void Guild::Member::SetStats(Player* player)
{
    ...
    for (PlayerSpellMap::const_iterator spellIter = player->GetSpellMap().begin(); spellIter != player->GetSpellMap().end(); ++spellIter)
    {
		...
		if(sSpellMgr->IsPrimaryProfessionSpell(spellInfo->Id))
        {
            uint32 skill = 0;
...
void Guild::UpdateMemberData(Player* plr, uint8 dataid, uint32 value)
{
    if (Member* pMember = GetMember(plr->GetGUID()))
    {
        ...
        {
            uint8 count_prof = 0;
            for (PlayerSpellMap::const_iterator spellIter = plr->GetSpellMap().begin(); spellIter != plr->GetSpellMap().end(); ++spellIter)
            {
                if(count_prof >= 2)
                    break;
				...
				if(sSpellMgr->IsPrimaryProfessionSpell(spellInfo->Id))
                {
                    ...
                    uint32 value = (uint32)plr->GetSkillValue(skill);
                    uint32 rank = sSpellMgr->GetSpellRank(spellIter->first);
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    _CreateRank(sObjectMgr->GetDarkCoreString(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetDarkCoreString(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetDarkCoreString(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetDarkCoreString(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetDarkCoreString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
